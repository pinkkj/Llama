...
"""
RETRO is not used for this run.

Megatron-LLaMA imports get_args_path unconditionally in megatron/arguments.py.
The original RETRO utils imports megatron (get_retro_args) which creates a circular
import when running tools/preprocess_data.py.

To unblock preprocessing/training without RETRO, provide only get_args_path.
"""
import os
from typing import Optional

def get_args_path(workdir: str, *args, **kwargs) -> Optional[str]:
    # Keep the same behavior as original: args.json inside retro workdir.
    return os.path.join(workdir, "args.json")
